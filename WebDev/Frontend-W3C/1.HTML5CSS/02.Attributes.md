...
Q1. Hey you!
  ...
  ```
    ...
    Ans: No
    Explanation
    You cannot have a paragraph element inside `<head>`. Elements that can be used inside `<head>` are `<title>`, `<base>`, `<link>`, `<style>`, `<meta>`, `<script>` and `<noscript>`.
...
Q2. Show me your id
    ...
    a. split by spaces if you want to specify multiple values
    b. the same across a given type of element i.e. all paragraphs have the same id
    c. unique to that element and can only be used once
    d. written in all caps (“ID=“)

    Ans: c
    Explanation
    The id of an element should be unique and can only be used once. Unlike the class attribute, you cannot have more than one id value for an element.
...
Q3. Tag specific?
    ...
    Ans: False
    Explication
    Non-global attributes can apply to more than one tag. The size attribute can be applied to <input> and <select> tags. Read more about them in the HTML attribute reference available at Mozilla Developer Network.
...
Q4. It's required!
    ...
    Ans: a, d
    Explanation
    The only two required attributes of an image tag are src and alt. All others are optional. src is used to specify the location the image should be fetched from and alt provides a short description of the image that is displayed for assistive technology users, search engines or if the image does not display.
...
Q5. Tooltip in title
    ...
    Title is a global attribute and so can be used on any element. Another good usage is to expand abbrievations. `<abbr title="HyperText Markup Language">HTML</abbr>`
...
Q6. Browser support?
    ...
    Ans: False
    Explication
    Not all browsers support all attributes. http://caniuse.com is a good resource to find out browser support for a particular attribute. Here is an example for the [download attribute](http://caniuse.com/#search=download%20attribute).

Q7. Valid id attribute?
...
Q8. Truly global
    ...
    a. All global attributes are of type boolean
    b. Global attributes are common to all elements
    c. Only 'id' and 'class' are global attributes
    d. Global attributes only apply to elements that do not have any attributes of their own

    Ans: b
    Explanation
    Global attributes are a list of attributes that are common to all elements in HTML.
...
Q9. Multiple classes
    ...
    Ans: b
    Explanation
    You can include two classnames for an element by separating them by space.
    <p class="poetry shakespeare">Insert Shakespeare poetry here</p>
    The paragraph above has two classnames - poetry and shakespeare, split by space.
...
Q10. Semantic elements

    True or False? Semantic elements, like article and section, don't use attributes.

    Ans: False
    Explanation
    Though semantic elements may not need an attribute to indicate its semantics, they can still use global attributes that apply to all elements


Q11. My text is important!
    ...
    Ans: b
    Explanation
    The strong element represents strong importance, seriousness, or urgency for its contents while emphasis is used to stress emphasis of its contents.
...
Q12. Mark my words
    ...
    Ans: d
    Explanation
    <mark> is used to highlight text. It will display with a yellow background in most browsers.


Q13. Non-semantic tags
    ...
    Ans: c
    Explanation
    Semantic elements suggest the content within tags. The <div> tag does not provide any information of the content within.


Q14. header and footer elements

    True or False? You can have multiple header and footer elements in your Web page.

    Ans: b
    Explication
    <header> and <footer> elements do what it is intended to for its parent element which can be a section. So each section in your Web page can have a header and footer just like each article, div, etc.


Q15. Who does semantic elements benefit?

    Who does semantic elements benefit?
    (select all that apply - 4 correct answers!)

    a. You
    b. Assistive technology users
    c. The browser
    d. The search engine
    e. The server
    ...
    Ans: a, b, c, d
    Explanation
    Semantic elements convey more information about your HTML document's content and structure which is going to be useful for you when you revisit your code after a long break. For the browser, it can better differentiate different types of data which results in better display of content in different devices. Assistive technology like screen readers gather more information from semantic HTML. It will read content marked up with <em> with emphasis or read header tags in a higher voice to indicate it is a heading. Better markup structure imrpoves the automated processing of documents in search engines.


Q16. Room with a view
    ...
    Ans: Explanation
    The details element contains <summary> and other content. Only summary will be displayed and it should be expanded to view all other content.
...
Q17. Image info
    ...
    Ans: e
    Explanation
    Title attribute creates a tooltip with additional information when you hover the mouse over the image. This should not be confused with the alt attribute which is a source of alternate information for the image.
...
Q18. Best image path
    ...
    a. package\assets\Images\example.PNG
    b. images/example.png
    c. Package/Assets/Images/Example.png
    d. images\example.png
...
Q19. Half image
    ...
    Ans: False
    Explication
    Specifying either one is sufficient and the aspect ratio will be maintained.
...
Q20. Adding images
    ...
    Ans: True
    Explication
    Other than the image tag, they can also be added through CSS which is the recommended way to add images that serve a decorative purpose.
...
Q22. What does the 'alt' attribute do?
    ...
    Ans: b
    Explanation
    The 'alt' attribute is used to provide a short description of the image or the function it serves for users who use assistive technology like screen readers, for search engines or if the image cannot be displayed for some reason. For a longer description, either add a link to the description after the image, describe the location of the longer description in the alt attribute, or use figure to structurally associate the image with its longer description.


Q23. Background image

    If you have an image you want to add as a background to your Web page, your 'alt' attribute's value should be:

    a. no alt attribute is needed
    b. empty - ""
...
Q24. Give me an alt!
    ...
    a. Image 3
    b. Boy looking at his school computer overjoyed and surprised by his exam results
    c. Surprised boy
    d. "" - no value

    Ans: b
    Explanation
    This looks like an informational image so some text that describes the information displayed visually is appropriate.
    Empty alt value is typically used for decorative images.


### Hyperlinks (25-28)

Q25. Download attribute: 

    What is the purpose of your download attribute's value in a hyperlink?

    a. It is a boolean value authorizing download
    b. It is the name of file in the location specified in href
    c. Downloaded file name will be modified according to value specified
    d. Download attribute has no value

    Ans: c
    Explanation
    The download attribute can have a value. If no value is specified, it downloads the file preserving the original file name. If a value is provided, it modifies the file name according to the value specified when downloading.


Q26. Linking documents: 
    ...
    Ans: d
    Explanation
    The anchor element is used to link different documents across the internet. There is no element called 'hyperlink'. 'src' and 'href' are attributes.
...
Q27. Anchor element: 
    ...
    Ans: False
    Explication
    While navigating to another page or part of your page is the typical use for an anchor element, the use of the download attribute introduced in HTML5 makes a link download a file instead of navigate to another location.


Q28. Hyperlink destination
    ...
    a. Opens the page in a separate frame
    b. Opens the page in a new tab or window
    c. Doesn't do anything as the code above is invalid
    d. Opens the page in the same tab or window

    Ans: b
    Explanation
    target value '_blank' will open in a new tab/window while '_self' opens in the same tab/window.
