...
+ Example: [player w/ CSS transformation](src/03c-example08.html)
  ...
  + array for possible setting to transform and set default one: `var properties = ['transform', 'WebkitTransform', 'MozTransform', 'msTransform', 'OTransform'], prop = properties[0];`
  + find the properties browser supported:
  ...
  + add event listener and switch to related cases: `controls.addEventListener('click', function(e) {...});`
    ...
    + reset button: `case 'reset': zoom = 1; rotate = 0; v.style.top = 0 + 'px'; v.style.left = 0 + 'px'; v.style[prop]='rotate('+rotate+'deg) scale('+zoom+')'; break;`
  + error handling for default: `e.preventDefault();`

+ Example: [display events and properties](src/03c-example09.html)
...
+ Example: [buffering status](src/03c-example10.html)
  ...
  + container to display the status: `<p id="loadStatus">MOVIE LOADING...</p>`
  ...
  + callback to calculate the percent of video loaded: `function getPercentProg() {...}`
