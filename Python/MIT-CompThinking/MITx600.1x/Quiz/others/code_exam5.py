...
class edx(object):
    ...
    def setGrade(self, grade, course="6.01x"):
        ...
        If `course` was not part of the initialization, then no grade is set, 
    ...
    def getGrade(self, course="6.02x"):
        """
        course: string 
        This method gets the grade in the the courseInfo object named by 
        `course`.
    ...
    def setPset(self, pset, score, course="6.00x"):
        ...
        If `course` is not part of the initialization, then no pset score is 
        set, and no error is thrown.
        """
        for c in self.myCourses:
            if c.courseName == course:
                c.setPset(str(pset), score)
